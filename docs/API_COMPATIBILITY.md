...
valid OIDC or first-party local account authentication is active. Filesystem mode
rejects a quarantine root that canonicalizes to the public object-store root. R2
mode rejects the same normalized endpoint, bucket, and prefix as the public object
store. Invalid, missing, or shared-store configuration fails startup instead of
exposing a weaker route surface. Setting the quarantine backend to `disabled`
restores the standard router without altering stored publication evidence.
